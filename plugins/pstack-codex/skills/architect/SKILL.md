...
Open a Codex plan with one entry per phase before starting. Autonomous mode without checkpoints needs the list to show phase position and keep phases from silently disappearing.
...
Use your configured architect-runners (defaults `gpt-5.6-sol` at `xhigh`, `gpt-5.6-terra` at `xhigh`, `gpt-5.6-luna` at `xhigh`, `gpt-5.5` at `xhigh`).

...
3. Pick the runners. Use `arena-runners` from `~/.codex/pstack-models.json` when present. Otherwise default to one each on `gpt-5.6-terra` at `xhigh`, `gpt-5.6-sol` at `max`, `gpt-5.6-luna` at `high`, `gpt-5.5` at `xhigh`. Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive.

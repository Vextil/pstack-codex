...
4. Pick the worker model from `swarm-workers` in `~/.codex/pstack-models.json` when present. Otherwise use `gpt-5.6-luna` at `xhigh`. For a model race, name each arm's model up front.

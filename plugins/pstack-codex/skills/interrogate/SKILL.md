...
Launch all reviewers back-to-back before waiting using Codex's collaboration tools. Use the `interrogate-reviewers` list from `~/.codex/pstack-models.json` when present, one reviewer per entry, extending or shrinking the Reviewer A/B/C/D labels below to the configured entry count; otherwise use the table defaults.

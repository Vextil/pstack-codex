...
- **A vague finish condition.** "make it better" gives a long-running task or heartbeat nothing to check. Give a command or artifact that can pass or fail.
...
- **Treating `auto` as a model slug.** `auto` and `inherit-parent` mean "omit the model field so the subagent inherits the parent task model." [Setup](./01-setup.md) covers the roles.
